...
//https://github.com/harryi3t/psx-download-helper-nodejs/pull/2/files
let dataDir = path.join(__dirname, "data"),
    cliArgs = process.argv.slice(2),
    dataDirOptionIndex = cliArgs.indexOf("--gamedata");

if (dataDirOptionIndex !== -1 && cliArgs[dataDirOptionIndex + 1]) {
    if (fs.existsSync(cliArgs[dataDirOptionIndex + 1])) {
        dataDir = path.join(cliArgs[dataDirOptionIndex + 1]);
    } else {
...
console.log(`Current Game Data Directory:- "${dataDir}"`);
...
function onRequest(client_req, client_res) {
    ...
    if (match = client_req.url.match(PS_REGEX)) {
        ...
        console.log('psxdh: Range ', client_req.headers.range, ' byte')
        ...
        fs.stat(filePath, function (err, stat) {
            ...
            console.log('psxdh: Found the game file locally. Transferring to console!')
